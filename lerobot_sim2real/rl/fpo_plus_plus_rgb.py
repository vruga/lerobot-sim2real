...
class PPOArgs:
    ...
    fpo_per_sample_clipping: bool = True       # Use per-sample ratio clipping (FPO++ improvement)
    fpo_asymmetric_trust_region: bool = True   # Use ASPO for asymmetric trust regions (FPO++ improvement)
...
class DiffusionPolicy(FeedForwardNN):
    ...
    def sample_action(self, state_norm: torch.Tensor) -> torch.Tensor:
        ...
        # Zero-sampling: initialize from ε=0 for deterministic test-time inference (Sec. III-D)
        x_t = torch.zeros(1, self.action_dim, device=self.device)

        # Standard forward Euler integration
        for step in range(num_steps):
            t_val = step / num_steps  # t ∈ [0, 1)
            ...
            x_t = x_t + dt * velocity  # standard Euler step

        return x_t[0]

    def sample_action_with_info(self, state_norm: torch.Tensor, num_train_samples: int = 16,
                                use_zero_init: bool = False):
        """
        Run Euler diffusion and compute CFM loss info for FPO++ training.

        Args:
            state_norm: [B, state_dim] or [state_dim]
            num_train_samples: Nmc — number of MC samples per action for CFM loss
            use_zero_init: if True, start Euler from ε=0 (zero-sampling for eval);
                           if False, start from ε~N(0,I) (stochastic training rollout)

        Returns:
            pred_action: [B, A] or [A] final denoised action
            x_t_path: [B, T+1, A] intermediate steps
            eps_sample: [B, N, A] MC noise samples (always random, for CFM training)
            t_samples: [B, N, 1] MC time samples
            initial_cfm_loss: [B, N] old-policy CFM losses (detached)
        """
        if state_norm.ndim == 1:
            state_norm = state_norm.unsqueeze(0)
        ...
        # Euler init: zero for eval (zero-sampling, Sec. III-D), random for training exploration
        if use_zero_init:
            x_t = torch.zeros(B, self.action_dim, device=self.device)
        else:
            x_t = torch.randn(B, self.action_dim, device=self.device)  # ε ~ N(0, I)

        x_t_path = [x_t.detach().clone()]

        # Standard forward Euler integration (no damping, no midpoint)
        for step in range(self.num_steps):
            t_val = step / self.num_steps  # t ∈ [0, 1)
        ...
        # MC samples for CFM loss: ε_i ~ N(0, I), τ_i ~ U[0, 1]  (Eq. 7-8)
        eps_sample = torch.randn(B, num_train_samples, self.action_dim, device=self.device)  # [B, N, A]
        t_samples = torch.rand(B, num_train_samples, 1, device=self.device)                 # [B, N, 1]
        ...
        initial_cfm_loss = self.compute_cfm_loss(
            ...
            t_samples.reshape(BN, -1),
        ).view(B, num_train_samples)  # [B, N]

        if B == 1:
            return x_t[0], x_t_path, eps_sample, t_samples, initial_cfm_loss.detach()
        return x_t, x_t_path, eps_sample, t_samples, initial_cfm_loss.detach()
    ...
    def compute_cfm_loss(self, state_norm: torch.Tensor,
        ...
        # Clamp t to open interval to avoid numerical issues at boundaries
        t = torch.clamp(t, 0.001, 0.999)

        # Linear interpolation: x_t^τ = τ·x1 + (1-τ)·ε  (Eq. 5)
        x_t = t * x1 + (1 - t) * eps  # [B, D_a]
        inp = torch.cat([state_norm, x_t, t], dim=1)  # [B, state_dim + D_a + 1]

        velocity_pred = self(inp)  # [B, D_a]

        # CFM target velocity: d/dτ x_t^τ = x1 - ε  (Eq. 6)
        target_velocity = x1 - eps  # [B, D_a]

        # Squared L2 loss, averaged over action dims (Eq. 8)
        loss = ((velocity_pred - target_velocity) ** 2).mean(dim=1)  # [B]

        # Clamp individual losses before differencing for stability (Appendix C.23)
        loss = torch.clamp(loss, 0.0, 50.0)
...
class Agent(nn.Module):
    ...
    def get_action(self, x, deterministic=False):
        ...
        # Always use zero-sampling at eval time (Sec. III-D)
        a, _, _, _, _ = self.actor_dp.sample_action_with_info(feats, num_train_samples=1, use_zero_init=True)
    ...
    def fpo_action_value_and_info(self, x):
        ...
        N = self.args.fpo_num_train_samples if self.args else 16
        # use_zero_init=False: stochastic rollout for exploration during training
        a_raw, _, eps, t_s, init_loss = self.actor_dp.sample_action_with_info(
            feats, num_train_samples=N, use_zero_init=False
        )
        # a_raw: unclipped Euler endpoint — used as x1 in CFM loss (both here and in update)
        # a: clipped for environment interaction only
        a = self._clip_to_action_space(a_raw)
        v = self.critic(feats)
        # Return a_raw so the update can compute new_cfm_loss with the same x1 as init_loss
        return a, v, feats, eps, t_s, init_loss, a_raw
...
def train(args: PPOArgs):
    ...
    # Extract base_camera_settings if present, as some environments don't accept it during initialization
    base_camera_settings = args.env_kwargs.get('base_camera_settings', None)
    # Create a copy of env_kwargs without base_camera_settings to avoid passing it to the environment constructor
    filtered_env_kwargs = {k: v for k, v in args.env_kwargs.items() if k != 'base_camera_settings'}
    ...
    if base_camera_settings is not None:
        if hasattr(eval_envs.unwrapped, 'base_camera_settings'):
            eval_envs.unwrapped.base_camera_settings = base_camera_settings
    ...
    # FPO++ rollout buffers — lazy-allocated on first step once sizes are known
    N = args.fpo_num_train_samples
    A = int(np.prod(envs.single_action_space.shape))
    fpo_eps      = None   # [T, B, N, A] — MC noise samples
    fpo_t        = None   # [T, B, N, 1] — MC time samples
    fpo_init     = None   # [T, B, N]   — old-policy CFM losses
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # ALGO LOGIC: action logic — use fpo_action_value_and_info to collect
            # the old-policy CFM losses and MC samples needed for FPO++ importance weights
            with torch.no_grad():
                action, value, _feats, eps_s, t_s, init_loss, a_raw = agent.fpo_action_value_and_info(next_obs)
                values[step] = value.view(-1)
            actions[step] = action          # clipped — sent to env
            actions_raw[step] = a_raw       # unclipped Euler endpoint — used as x1 in CFM loss
            logprobs[step] = torch.zeros(args.num_envs, device=device)  # dummy; FPO uses CFM losses

            # Lazy-allocate FPO buffers once we know the action/sample sizes
            if fpo_eps is None:
        ...
        b_actions_raw = actions_raw.reshape((-1,) + envs.single_action_space.shape)  # unclipped
        b_advantages = advantages.reshape(-1)
        b_returns = returns.reshape(-1)
        b_values = values.reshape(-1)
        # Flatten FPO++ rollout buffers
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Build dict obs for this minibatch
                mb_obs = b_obs[mb_inds]  # plain dict {k: tensor[mb_inds]}

                mb_actions     = b_actions[mb_inds]      # [B, A] — clipped (env action)
                mb_actions_raw = b_actions_raw[mb_inds]  # [B, A] — unclipped Euler endpoint
                mb_advantages  = b_advantages[mb_inds]   # [B]
                mb_values      = b_values[mb_inds]       # [B]
                mb_returns     = b_returns[mb_inds]      # [B]

                # Retrieve stored rollout-time MC samples and old-policy losses
                eps_mb  = b_eps[mb_inds]   # [B, N, A]
                t_mb    = b_t[mb_inds]     # [B, N, 1]
                init_mb = b_init[mb_inds]  # [B, N] — old-policy CFM loss (frozen)

                # Forward pass through current policy (with gradients)
                feats    = agent.get_features(mb_obs)  # [B, L]
                newvalue = agent.critic(feats).view(-1) # [B]

                B  = feats.size(0)
                L  = feats.size(1)
                A_ = mb_actions_raw.size(1)
                BN = B * N

                # Expand to [B*N, ...] and recompute CFM loss with current policy weights
                # using the SAME (ε, τ) pairs and the SAME x1 (unclipped) as during rollout
                feats_bn = feats.unsqueeze(1).expand(B, N, L).reshape(BN, L)
                x1_bn    = mb_actions_raw.unsqueeze(1).expand(B, N, A_).reshape(BN, A_)  # unclipped
                eps_bn   = eps_mb.reshape(BN, A_)   # [B*N, A]
                t_bn     = t_mb.reshape(BN, 1)      # [B*N, 1]

                new_cfm_loss = agent.compute_cfm_loss(feats_bn, x1_bn, eps_bn, t_bn).view(B, N)  # [B, N]

                # Per-sample FPO++ ratio (Eq. 10): ρ_i = exp(ℓ_old_i − ℓ_new_i)
                # Both losses are already clamped by compute_cfm_loss (Appendix C.23 step 1)
                logratio = init_mb - new_cfm_loss                                          # [B, N]
                logratio = torch.clamp(logratio, -args.fpo_logratio_clip, args.fpo_logratio_clip)  # step 2
                ratio    = logratio.exp()                                                  # [B, N]

                # Advantage normalisation (applied before expanding to per-sample)
                mb_adv_norm = mb_advantages
                if args.norm_adv:
                    mb_adv_norm = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)
                if args.positive_advantage:
                    mb_adv_norm = torch.nn.functional.softplus(mb_adv_norm)

                expanded_advantages = mb_adv_norm.unsqueeze(1).expand(-1, N)  # [B, N]

                # FPO++ objective (Eq. 13): sum over Nmc per-sample ASPO terms
                pg_loss = compute_fpo_plus_plus_objective(
                    ratio=ratio,
                    advantage=expanded_advantages,
                    clip_coef=args.clip_coef,
                    per_sample_clipping=args.fpo_per_sample_clipping,
                    asymmetric_trust_region=args.fpo_asymmetric_trust_region,
                )
                pg_loss = pg_loss.mean(dim=1).mean()

                # Track clip fraction
                clipfracs += [((ratio - 1.0).abs() > args.clip_coef).float().mean().item()]

                # Approximate KL for early stopping (standard PPO estimator)
                with torch.no_grad():
                    approx_kl = ((ratio - 1) - logratio).mean().item()

                # Inner-loop early stopping: break minibatch loop immediately
                if args.target_kl is not None and approx_kl > args.target_kl:
                    kl_exceeded = True
                    break

                # Value loss with clipped objective
                if args.clip_vloss:
        ...
        if logger is not None:
            ...
            logger.add_scalar("losses/total_loss", loss.item(), global_step)
